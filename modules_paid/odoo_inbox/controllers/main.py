...
class WebsiteOdooInbox(http.Controller):
    ...
    def _render_odoo_message(self, domain=[], link='/mail', page=1, label=None, color='bluecolor', search=None, existing_tag=None, existing_folder=None):
        ...
        inbox_domain = counter_domain + [('msg_unread', '=', False), ('message_label', 'in', ['inbox', 'starred']), ('mail_folder_id', '=', False)]
        ...
        folder_domain = counter_domain + [('mail_folder_id', '=', existing_folder)]
        ...
        for fid in folder_ids.ids:
            ffolder_domain = counter_domain + [('msg_unread', '=', False), ('mail_folder_id', '=', fid)]
            ct = len(self.get_message_counter_domain(MailMessage, ffolder_domain))
            counter_fd_msgs.update({str(fid): str(ct)})

        total = 0
        if label == 'inbox':
            tinbox_domain = counter_domain + [('message_label', 'in', ['inbox', 'starred']), ('mail_folder_id', '=', False)]
    ...
    def odoo_message_read(self, **kw):
        ...
        inbox_domain = domain + [('msg_unread', '=', False), ('message_label', 'in', ['inbox', 'starred']), ('mail_folder_id', '=', False)]
        inbox_mssg_count = len(self.get_message_counter_domain(MailMessage, inbox_domain))
        starred_domain = domain + [('msg_unread', '=', False), ('message_label', '=', 'starred')]
        starred_mssg_count = len(self.get_message_counter_domain(MailMessage, starred_domain))
        snoozed_domain = domain + [('msg_unread', '=', False), ('message_label', '=', 'snoozed')]
        snoozed_mssg_count = len(self.get_message_counter_domain(MailMessage, snoozed_domain))
        folder_domain = domain + [('msg_unread', '=', False), ('mail_folder_id', '!=', False)]
        folder_mssg_count = len(self.get_message_counter_domain(MailMessage, folder_domain))

        folder_ids = request.env['message.folder'].sudo().search([('user_id', '=', user_id.id)])
        counter_fd_msgs = {}
        for fid in folder_ids.ids:
            ffolder_domain = domain + [('msg_unread', '=', False), ('mail_folder_id', '=', fid)]
            ct = len(self.get_message_counter_domain(MailMessage, ffolder_domain))
            counter_fd_msgs.update({str(fid): str(ct)})
    ...
    def message_post_send(self, **post):
        ...
        if messageObj.author_id:
            ...
            mailObj = request.env['mail.mail'].create({
            ...
            message = message_object.message_post(
                    ...
                    attachment_ids=mailObj.mail_message_id.attachment_ids,
    ...
    def odoo_all_mssg_trash(self, messg_ids, **post):
        for mssg in messg_ids:
            message_id = request.env['mail.message'].sudo().browse(int(mssg))
            if message_id or message_id.mail_folder_id:
                message_id.write({'mail_folder_id': False,
                                  'message_label': 'trash'
                                  })
        return True
    ...
    def odoo_message_tag_assign(self, message_id, tag_ids=[], create_tag_input=None, **post):
        if message_id:
            ...
            message_tag_list_template = request.env.ref('odoo_inbox.message_tag_list')._render({'mail_message': message})
            message_tag_dropdown = request.env.ref('odoo_inbox.tag_dropdown')._render({'mail_message': message, 'tag_ids': main_tag_ids})
    ...
    def odoo_folders(self, folder, page=1, **kw):
        domain = [('mail_folder_id', '=', folder.id)]
    ...
    def odoo_folder_edit(self, **kw):
        if kw.get('mail_folder_id') and kw.get('folder_name'):
            mail_folder_id = request.env['message.folder'].sudo().browse(int(kw.get('mail_folder_id')))
            mail_folder_id.name = kw.get('folder_name')
        return request.redirect(request.httprequest.referrer or '/mail/inbox')

    @http.route(['/mail/folder_delete'], type='http', auth="user", method=['POST'], website=True)
    def odoo_folder_delete(self, **kw):
        if kw.get('mail_folder_id'):
            mail_folder_id = request.env['message.folder'].sudo().browse(int(kw.get('mail_folder_id')))
            mail_folder_id.unlink()
        return request.redirect('/mail/inbox')
    ...
    def odoo_move_to_folder(self, folder, message, **kw):
        if folder and message:
            message.mail_folder_id = folder.id
        return request.redirect(request.httprequest.referrer or '/mail/inbox')

    @http.route('/mail/all_move_to_folder', type="json", auth="user", website=True)
    def odoo_all_move_to_folder(self, mail_folder_id, messg_ids, **post):
        for mssg in messg_ids:
            message_id = request.env['mail.message'].sudo().browse(int(mssg))
            if mail_folder_id == 'move_to_inbox':
                message_id.write({'mail_folder_id': False, 'message_label': 'inbox',
                                  })
            elif mail_folder_id and message_id:
                message_id.mail_folder_id = mail_folder_id
        return True
    ...
    def odoo_new_folder(self, **kw):
        if kw.get('create_folder'):
            ...
            mail_folder_id = request.env['message.folder'].create({'name': kw.get('create_folder'),
                                                              'user_id': user_id})
            if kw.get('message_id') and mail_folder_id:
                message_id = request.env['mail.message'].sudo().browse(int(kw.get('message_id')))
                message_id.mail_folder_id = mail_folder_id.id
        return request.redirect(request.httprequest.referrer or '/mail/inbox')
