...
class FetchmailServer(models.Model):
    ...
    server = fields.Char(string='Server Name', related='mail_server_id.server', help="Hostname or IP of the mail server")
    ...
    def fetch_mail(self):
        ...
        for server in self:
            if server.type == 'imap' and server.last_internal_date:
                _logger.info('start checking for new emails, starting from %s on %s server %s',server.last_internal_date, server.type, server.name)
                ...
                try:
                    ...
                    _logger.exception("General failure when trying to fetch mail by date from %s server %s.",server.type,server.name)
                finally:
                    if imap_server:
                        imap_server.close()
                        imap_server.logout()
                ...
                if last_date:
                    _logger.info("Fetched %d email(s) on %s server %s, starting from %s; %d succeeded, %d failed.", count,
