...
class StyleThree(models.AbstractModel):
    _name = 'report.dev_product_catalog_report.style_three_template'
    ...
    def get_products(self, popup_id):
        if popup_id.catalog_type == 'product':
            if popup_id.box_per_row == '3':
                if popup_id.print_using == 'template':
                    chunks = [popup_id.product_template_ids.ids[i:i + 3] for i in range(0, len(popup_id.product_template_ids.ids), 3)]
                if popup_id.print_using == 'variant':
                    chunks = [popup_id.product_ids.ids[i:i + 3] for i in range(0, len(popup_id.product_ids.ids), 3)]
            if popup_id.box_per_row == '2':
                if popup_id.print_using == 'template':
            ...
            if all_products:
                ...
                if popup_id.box_per_row == '3':
                    chunks = [all_products[i:i + 3] for i in range(0, len(all_products), 3)]
                if popup_id.box_per_row == '2':
